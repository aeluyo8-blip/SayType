...
从 [GitHub Releases](../../releases) 下载 `phone-type-xxx.apk` 装到手机
...
1. 手机连上和电脑**同一个 WiFi**，打开 phone-type APP
...
协议细节见 [CLAUDE.md](CLAUDE.md)。PC 端零原生依赖，只用 Node 20+ 和 `ws`。
...
# PC 端
npm install
npm start            # 或双击 启动服务.bat
npm test             # 单元测试
node scripts/verify-pc.mjs   # 端到端验证（自动开记事本验证中文注入）
...
- `启动服务.bat` + `scripts/start-service.ps1`：小白启动入口（检测 Node / 装依赖 / 放行防火墙 / 启动服务）
- `scripts/dev-client.mjs`：命令行模拟手机发文字（`node scripts/dev-client.mjs 127.0.0.1 8787 <PIN> "测试"`）
- `scripts/verify-pc.mjs`：端到端验证脚本
